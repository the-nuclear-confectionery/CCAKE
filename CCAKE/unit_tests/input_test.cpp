...
TEST (InputTest, Constructor){
  ...
  EXPECT_FALSE( in.settingsPtr->input_as_entropy );

  //Simulation parameters
  EXPECT_DOUBLE_EQ( in.settingsPtr->dt, 0.05 );
  EXPECT_DOUBLE_EQ( in.settingsPtr->hT, 0.5 );
  EXPECT_DOUBLE_EQ( in.settingsPtr->hEta, 0.5 );
  EXPECT_EQ( in.settingsPtr->kernel_type, "cubic_spline" );
  EXPECT_DOUBLE_EQ( in.settingsPtr->e_cutoff, .15 );
  EXPECT_TRUE( in.settingsPtr->buffer_event );
  EXPECT_TRUE (in.settingsPtr->circular_buffer);
  EXPECT_DOUBLE_EQ( in.settingsPtr->padding_thickness, .1);
  ...
  EXPECT_FALSE( in.settingsPtr->particlization_enabled);
  EXPECT_EQ( in.settingsPtr->Freeze_Out_Type, "fixed_T" );
  EXPECT_DOUBLE_EQ( in.settingsPtr->Freeze_Out_Temperature, 150. );

  //Hydro values
  EXPECT_TRUE( in.settingsPtr->baryon_charge_enabled );
  EXPECT_TRUE( in.settingsPtr->strange_charge_enabled );
  EXPECT_TRUE( in.settingsPtr->electric_charge_enabled );
...
particlization:
  enabled: false # Set to true to output particlization hypersurface
