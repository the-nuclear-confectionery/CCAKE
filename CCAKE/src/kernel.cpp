...
template<> const double SPHkernel<1>::knorm  = 2./3.; //Used for q < 1 
template<> const double SPHkernel<1>::knorm2 = 1./6.; //Used for 1 < q < 2
template<> const double SPHkernel<1>::kgrad  = -1./2.; 
template<> const double SPHkernel<1>::kgrad2 = 1.; 
...
template<> const double SPHkernel<2>::kgrad2 = 15./(7.*M_PI);

template<> const double SPHkernel<3>::knorm  = 1./M_PI; 
template<> const double SPHkernel<3>::knorm2 = 1./(4*M_PI); 
template<> const double SPHkernel<3>::kgrad  = -3./(4.*M_PI); 
template<> const double SPHkernel<3>::kgrad2 = 3./(2.*M_PI); 
...
/// @param rel_dist An array containing the relative distance between two particles, i.e. ra-rb
