...
// 
  ...
  // }
  template<>
  double kernel<2>(double distance, const double  hT )
  {
    double const knorm       = 10.0/(7.*pi*hT*hT);
    double const knorm2      = knorm*0.25;
  ...
  template<>
  double kernel<1>(double distance, const double hT ){return 0;}
  template<>
  double kernel<3>(double distance, const double hT ){return 0;}
  ...
  {
    double const kgrad       = -15.0/(14.0*pi*h*h*h);
    double const kgrad2      = 10.0/(7.0*pi*h*h*h*h);
    double q = r/h;
