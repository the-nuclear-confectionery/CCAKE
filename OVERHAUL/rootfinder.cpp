...
//std::cout << "Made it to line " << __LINE__ << " with status = " << status << std::endl;
    ...
    // test both conditions at once
    if ( gsl_multiroot_test_residual(solver->f, 1e-20) == GSL_CONTINUE
          || gsl_multiroot_test_delta(solver->dx, solver->x, 1e-20, 1e-20) == GSL_CONTINUE )
      status = GSL_CONTINUE;
