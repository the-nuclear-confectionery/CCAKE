...
void EoM_default<D>::reset_pi_tensor(std::shared_ptr<SystemState<D>> sysPtr)
{
  ...
  { 
    ...
    //diffusion
    milne::Matrix<double,3,4> q_matrix;
...
void EoM_default<D>::evaluate_time_derivatives( std::shared_ptr<SystemState<D>> sysPtr, std::shared_ptr<Settings> settingsPtr)
{
   ...
   auto fill_auxiliary_variables = KOKKOS_LAMBDA(int const is, int const ia){
    ...
    if(using_shear){ 
      ...
      F_extensive_bulk += -( lambda_Pipi*F_shv_nabla_u
                      - phi3*milne::contract(shv_cov,shv))/(sigma*gamma*tau_Pi);
  ...
  auto compute_charge_derivative = KOKKOS_LAMBDA(const int is, const int ia)
  {
    ...
    //declare caches
    milne::Vector<double,3> rho_ext;
    rho_ext = {rhoB_ext, rhoS_ext, rhoQ_ext};
    milne::Vector<double,3> grad_qa, div_qa;
    milne::Vector<double,3> F_extensive_N;
    milne::Matrix<double,3,4> q;
    milne::Matrix<double,D,3> M_extensive_N;
   ...
   if(using_diffusion){
    ...
    for(int icharge=0; icharge<3; ++icharge){
      ...
      F_extensive_N(icharge) += -device_hydro_vector.access(is, ia, hydro_info::F_q0_a, icharge)
                                 + grad_qa(icharge) -div_qa(icharge)- q(icharge,0)/t;
  ...
  { 
      ...
      double F_bulk = sigma*F_extensive_bulk - bulk*(gamma*divV + gamma/t + geometric_factor)/gamma;
    ...
    //aux MRFs 
     ...
     //print RI
     //std::cout << "   = "  << RI(1,0) << " " << RI(1,1) << " " << RI(1,2) << std::endl;
     //std::cout << "RI = "  << RI(0,0) << " " << RI(0,1) << " " << RI(0,2) << std::endl;
     //std::cout << "   = "  << RI(2,0) << " " << RI(2,1) << " " << RI(2,2) << std::endl;
  ...
  auto compute_derivatives = KOKKOS_LAMBDA(const int is, const int ia)
  {
    ...
    milne::Matrix<double,3,3> d_extensive_q_dt;
    milne::Matrix<double,3,3> F_q_ib;
    milne::Matrix3D<double,3,3,3> M_q_ibj;
    milne::Matrix3D<double,3,3,3> R_q_ibc;
    ...
    //  std::cout << "dN_dt: " << dN_dt(0) << " " << dN_dt(1) << " " << dN_dt(2) << std::endl;
    //  std::cout << "R*DN: " << milne::contract(R_extensive_entropy,dN_dt) << std::endl;
    //}
    ...
    //std::cout << "R_extensive_entropy: " << R_extensive_entropy(0) << " " << R_extensive_entropy(1) << " " << R_extensive_entropy(2) << std::endl;
    //std::cout << "milne::contract(R_extensive_entropy,dN_dt)" << milne::contract(R_extensive_entropy,dN_dt) << std::endl;
    //std::cout << "milne::contract(M_extensive_entropy,du_dt)" << milne::contract(M_extensive_entropy,du_dt) << std::endl;
...
template <unsigned int D>
void EoM_default<D>::calculate_MRF_shear(std::shared_ptr<SystemState<D>> sysPtr,bool using_vorticity)
{
  double t = (sysPtr->t);
  double t2 = t*t;
  double use_vort = 0.;
  ...
  auto compute_MRF_shear = KOKKOS_LAMBDA(const int is, const int ia)
  {
    ...
    //shv
    milne::Vector<double, D> shvilambda_shv0lambda;
    for(int idir=0; idir<D; ++idir){
    ...
    for(int idir=0; idir<D; ++idir){
      ...
      F_i0_dsigma(idir) = -(v_gradu_shv(idir)-grad_u_g_shv_i(idir)
                          ...
                          +( gamma*contra_metric_diag(idir+1)*shv_hybrid(0,3)*delta_i_eta(idir)/t
                            +gamma*shv(3,0)*delta_i_eta(idir)/t
      ...
      F_i0_domega(idir) = -(v_gradu_shv(idir)-grad_u_g_shv_i(idir)
                          ...
                          +(gamma*contra_metric_diag(idir+1)*shv_hybrid(0,3)*delta_i_eta(idir)/t
                            -gamma*shv(3,0)*delta_i_eta(idir)/t )/4.;
      ...
      if(D!=2){
        F_i0_dsigma(idir) += ( t*u(D-1)*delta_i_eta(idir)*contra_metric_diag(idir+1)*shv_hybrid(0,0)
                               +u(D-1)*shv_hybrid(idir+1,3)*gamma/t/4.  + shv_hybrid(idir+1,3)*t*u(D-1)
                               +shv(0,0)*delta_i_eta(idir)*u(D-1)/t
                               +t*u(D-1)*u(D-1)*(gamma*shv_hybrid(idir+1,0) + u(idir)*shv_hybrid(0,0))
                               +2.*gamma*u(D-1)*(gamma*shv_hybrid(idir+1,3)+u(idir)*shv(0,3))/t)/4.
                               + gamma*u(idir)*(shv_hybrid(3,0)*u(D-1)*t+ u(D-1)*shv_hybrid(0,3)/t)/3.;    

        F_i0_domega(idir) +=   (t*u(D-1)*delta_i_eta(idir)*contra_metric_diag(idir+1)*shv_hybrid(0,0)
                               +u(D-1)*shv_hybrid(idir+1,3)*gamma/t - shv_hybrid(3,idir+1)*t*u(D-1)
                               -shv(0,0)*delta_i_eta(idir)*u(D-1)/t
                               +t*u(D-1)*u(D-1)*(gamma*shv_hybrid(idir+1,0) + u(idir)*shv_hybrid(0,0))
                               +2.*gamma*u(D-1)*(gamma*shv_hybrid(idir+1,3)+u(idir)*shv(0,3))/t)/4.;        
      }
    ...
    milne::Matrix<double,2,3> shv_mu_lambda_shv_nu_lambda;
    for(int idir=0; idir<2; ++idir){
      for(int jdir=idir; jdir<3; ++jdir){
    ...
    for(int idir=0; idir<2; ++idir){
      for(int jdir=idir; jdir<3; ++jdir){
        ...
        F_domega(idir,jdir) = -( -grad_u_g_j_shv_i(idir,jdir) + shv_i_grad_uj(idir,jdir)
                              ...
                              +(t*fixed_size_u(2)*(contra_metric_diag(idir+1)*shv_hybrid(jdir+1,0)*delta_i_eta(idir)
                                ...
                                +gamma*(contra_metric_diag(idir+1)*shv_hybrid(jdir+1,3)*delta_i_eta(idir)
                                +contra_metric_diag(jdir+1)*shv_hybrid(idir+1,3)*delta_i_eta(jdir))/t
                                +fixed_size_u(2)*fixed_size_u(2)*t*(fixed_size_u(idir)*shv_hybrid(jdir+1,0)
                                +fixed_size_u(jdir)*shv_hybrid(idir+1,0))
        ...
        F_dsigma(idir,jdir) = -( -grad_u_g_j_shv_i(idir,jdir) - shv_i_grad_uj(idir,jdir)
                              ...
                              +(t*fixed_size_u(2)*(contra_metric_diag(idir+1)*shv_hybrid(jdir+1,0)*delta_i_eta(idir)
                                ...
                                +fixed_size_u(jdir)*shv_hybrid(idir+1,3))/t
                                +gamma*(shv(3,idir+1)*delta_i_eta(jdir)+shv(3,jdir+1)*delta_i_eta(idir))/t
                                +fixed_size_u(2)*(shv(0,idir+1)*delta_i_eta(jdir)+shv(0,jdir+1)*delta_i_eta(idir))/t
      ...
      F_dsigma(idir,idir) += -contra_metric_diag(idir+1)*shv_grad_u/3.
                             -contra_metric_diag(idir+1)*shv_hybrid(0,0)*geometric_factor/3.
                             -contra_metric_diag(idir+1)*(shv_hybrid(3,0)*fixed_size_u(2)*t
...
template <unsigned int D>
void EoM_default<D>::calculate_MRF_diffusion(std::shared_ptr<SystemState<D>> sysPtr)
{
  ...
  //auxiliary metric variables
  milne::Vector<double,D> delta_i_eta = milne::delta_i_eta<D>();
  milne::Vector<double,3> full_delta_i_eta = milne::delta_i_eta<3>();
  milne::Vector<double,4> contra_metric_diag = milne::get_contra_metric_diagonal<3>(t);
  ...
  auto compute_MRF_diffusion = KOKKOS_LAMBDA(const int is, const int ia)
  {
    ...
    milne::Vector<double,3> extensive_rho = {rhoB, rhoS, rhoQ};
    milne::Vector<double,D> u, u_cov;
    //fixed size
    milne::Vector<double,3> M_S;

    milne::Vector<double,3> R_S;
    milne::Vector<double,3> fixed_size_u;
    milne::Vector<double,3> div_qa; 
    milne::Vector<double,3> grad_qa; 
    milne::Matrix<double,3,3> grad_alpha_a;  
    milne::Matrix<double, 3, 3>  tau_q; 
    milne::Matrix<double, 3, 3>  kappa_q;
    milne::Matrix<double,3,4> q;

    //fill by hand
    milne::Vector<double,3> dalpha_i_ds = {dalpha_Bds, dalpha_Sds, dalpha_Qds};
    milne::Matrix<double, 3, 3> dalpha_i_drho_j;
    ...
    milne::Vector<double,3> F_0_D_b;
    milne::Vector<double,3> F_0_alpha_b;
    milne::Vector<double,3> A_0_alpha_b;    
    milne::Matrix<double,3,3> R_0_alpha_bc;
    milne::Matrix<double,3,D> M_0_alpha_bj;
    milne::Matrix<double,3,D> M_0_D_bj;

    //0 quantites final
    milne::Vector<double,3> F_0_q_b;
    milne::Matrix<double,3,3> R_0_q_bc;
    milne::Matrix<double,3,D> M_0_q_bj;


    milne::Matrix<double,3,3> F_alpha_ib;
    milne::Matrix<double,3,3> F_D_ib;
    milne::Matrix<double,3,3> A_alpha_ib;
    milne::Matrix3D<double,3,3,3> M_D_ibj;
    milne::Matrix3D<double,3,3,3> M_alpha_ibj;
    milne::Matrix3D<double,3,3,3> R_alpha_ibc;
    ...
    milne::Matrix<double,3,4> q_cov;
    //aux diffusion for each charge (only space part)
    milne::Vector<double,3> q_cov_aux_b;
    milne::Vector<double,3> q_cov_aux_q;
    milne::Vector<double,3> q_cov_aux_s;
    for(int idir=0; idir<3; ++idir){
    ...
    //fixed size u cov
    milne::Vector<double,3> fixed_size_u_cov = fixed_size_u;
    fixed_size_u_cov.make_covariant(t2);


    //invert the relaxation time matrix
    milne::Matrix<double,3,3> inv_tau_q = milne::inverse(tau_q);
    milne::Matrix<double,3,3> inv_tau_tau;
    milne::Matrix<double,3,3> inv_tau_kappa;
    for(int icharge=0; icharge<3; ++icharge){
      for(int jcharge=0; jcharge<3; ++jcharge){
    ...
    for(int icharge=0; icharge<3; ++icharge){    
      ...
      for(int jcharge=0; jcharge<3; ++jcharge){
        F_0_q_b(icharge) += (-inv_tau_tau(icharge,jcharge)*F_0_D_b(jcharge)
                              ...
                              +inv_tau_kappa(icharge,jcharge)*A_0_alpha_b(jcharge)*(gamma*divV+geometric_factor+gamma/t)
                              )/(gamma)
                            -inv_tau_tau(icharge,jcharge)*q(jcharge,0)*(gamma*divV+geometric_factor+gamma/t)/gamma;
      ...
      for(int icharge=0; icharge<3; ++icharge){
        ...
        for(int jcharge=0; jcharge<3; ++jcharge){
          F_q_ib(idir,icharge) += (-inv_tau_tau(icharge,jcharge)*F_D_ib(idir,jcharge)
                                  ...
                                  +inv_tau_kappa(icharge,jcharge)*A_alpha_ib(idir,jcharge)*(gamma*divV+geometric_factor+gamma/t)
                                  )/(gamma*sigma);
