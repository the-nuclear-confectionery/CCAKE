...
//   k4 = Cabana::AoSoA<EvolverCache, DeviceType, VECTOR_LENGTH>("k1", n_particles);
  auto kbigshv = Cabana::slice<evolver_cache_info::big_shear>(k);
  ...
  auto krhoB = Cabana::slice<evolver_cache_info::specific_baryon>(k);
  auto krhoS = Cabana::slice<evolver_cache_info::specific_strangeness>(k);
  auto krhoQ = Cabana::slice<evolver_cache_info::specific_electric>(k);
  auto kbigBulk = Cabana::slice<evolver_cache_info::big_bulk>(k);
...
void Evolver<D>::set_current_timestep_quantities()
{
  ...
  auto bigshv = Cabana::slice<evolver_cache_info::big_shear>(evolver_cache);
  ...
  auto bigBulk = Cabana::slice<evolver_cache_info::big_bulk>(evolver_cache);
  auto rhoB = Cabana::slice<evolver_cache_info::specific_baryon>(evolver_cache);
  auto rhoS = Cabana::slice<evolver_cache_info::specific_strangeness>(evolver_cache);
  auto rhoQ = Cabana::slice<evolver_cache_info::specific_electric>(evolver_cache);
  ...
  auto fill_cache = KOKKOS_LAMBDA (const int is, const int ia)
  {
    for (int i=0; i<2; i++){
    for (int j=i; j<3; j++){
      bigshv.access(is, ia,i,j) = device_hydro_shear_aux_vector.access(is, ia, hydro_info::bigshv, i, j);
      //std::cout << "bigshv: " << bigshv.access(is, ia,i,j) << std::endl;
    }}
    ...
    bigBulk.access(is, ia) = device_hydro_scalar.access(is, ia, hydro_info::bigBulk);
    rhoB.access(is, ia) = device_specific_density.access(is, ia, densities_info::rhoB);
    rhoS.access(is, ia) = device_specific_density.access(is, ia, densities_info::rhoS); 
    rhoQ.access(is, ia) = device_specific_density.access(is, ia, densities_info::rhoQ);
...
void Evolver<D>::update_k(int n)
{
  ...
  auto d_bigshv_dt = Cabana::slice<evolver_cache_info::big_shear>(k);
  auto du_dt = Cabana::slice<evolver_cache_info::four_velocity>(k);
  auto dr_dt = Cabana::slice<evolver_cache_info::position>(k);
  auto dNb_dt = Cabana::slice<evolver_cache_info::specific_baryon>(k);
  auto dNs_dt = Cabana::slice<evolver_cache_info::specific_strangeness>(k);
  auto dNe_dt = Cabana::slice<evolver_cache_info::specific_electric>(k);
  auto ds_dt = Cabana::slice<evolver_cache_info::specific_entropy>(k);
  auto dbigBulk_dt = Cabana::slice<evolver_cache_info::big_bulk>(k);
  ...
  auto update_k_cache = KOKKOS_LAMBDA (const int is, const int ia)
  {
    for (int i=0; i<2; i++)
    for (int j=i; j<3; j++)
      Kokkos::atomic_add(&d_bigshv_dt.access(is, ia,i,j), w.at(n)*device_hydro_shear_aux_vector.access(is, ia, hydro_info::d_bigshv_dt, i, j));
    ...
    Kokkos::atomic_add(&dNb_dt.access(is, ia), w.at(n)*device_d_dt_spec.access(is, ia, densities_info::rhoB));
    Kokkos::atomic_add(&dNs_dt.access(is, ia), w.at(n)*device_d_dt_spec.access(is, ia, densities_info::rhoS));
    Kokkos::atomic_add(&dNe_dt.access(is, ia), w.at(n)*device_d_dt_spec.access(is, ia, densities_info::rhoQ));
    Kokkos::atomic_add(&ds_dt.access(is, ia), w.at(n)*device_d_dt_spec.access(is, ia, densities_info::s));
    Kokkos::atomic_add(&dbigBulk_dt.access(is, ia), w.at(n)*device_hydro_scalar.access(is, ia, hydro_info::dbigBulk_dt));
...
void Evolver<D>::update_rk4(double dt){
  ...
  //Create views for the device
      CREATE_VIEW(device_, systemPtr->cabana_particles);
      auto slice_bigshv0 = Cabana::slice<evolver_cache_info::big_shear>(evolver_cache);
      ...
      auto slice_rhoB0 = Cabana::slice<evolver_cache_info::specific_baryon>(evolver_cache);
      auto slice_rhoS0 = Cabana::slice<evolver_cache_info::specific_strangeness>(evolver_cache);
      auto slice_rhoQ0 = Cabana::slice<evolver_cache_info::specific_electric>(evolver_cache);
      auto slice_s0 = Cabana::slice<evolver_cache_info::specific_entropy>(evolver_cache);
      auto slice_bigBulk0 = Cabana::slice<evolver_cache_info::big_bulk>(evolver_cache);
      auto slice_E0 = Cabana::slice<evolver_cache_info::E0>(evolver_cache);
      //k<var> corresponds to w1k1 + 2*w2k2 + 2*w3k3 + w4k4 in RK4
      auto slice_kbigshv = Cabana::slice<evolver_cache_info::big_shear>(k);
      auto slice_ku = Cabana::slice<evolver_cache_info::four_velocity>(k);
      auto slice_kr = Cabana::slice<evolver_cache_info::position>(k);
      auto slice_krhoB = Cabana::slice<evolver_cache_info::specific_baryon>(k);
      auto slice_krhoS = Cabana::slice<evolver_cache_info::specific_strangeness>(k);
      auto slice_krhoQ = Cabana::slice<evolver_cache_info::specific_electric>(k);
      auto slice_ks = Cabana::slice<evolver_cache_info::specific_entropy>(k);
      auto slice_kbigBulk = Cabana::slice<evolver_cache_info::big_bulk>(k);
      ...
      auto update_values = KOKKOS_LAMBDA(const int is, const int ia){
        ...
        double specific_rhob0 = slice_rhoB0.access(is, ia);
        double k_rhoB = slice_krhoB.access(is, ia);
        double specific_rhos0 = slice_rhoS0.access(is, ia);
        double k_rhoS = slice_krhoS.access(is, ia);
        double specific_rhoQ0 = slice_rhoQ0.access(is, ia);
        double k_rhoQ = slice_krhoQ.access(is, ia);
        double bigBulk0 = slice_bigBulk0.access(is, ia);
        double kbigBulk = slice_kbigBulk.access(is, ia);
        double particles_E0 = slice_E0.access(is, ia);
        double kE0 = slice_kE0.access(is, ia);

        milne::Matrix<double, 2, 3> bigshv0, kbigshv;
        ...
        for(int idir=0; idir<2; ++idir){
          for(int jdir=idir; jdir<3; ++jdir){
            bigshv0(idir, jdir) = slice_bigshv0.access(is, ia, idir, jdir);
            kbigshv(idir, jdir) = slice_kbigshv.access(is, ia, idir, jdir);
          }
        }
        
        //Sum everything up
        double specific_s        = specific_s0         + dt*ks;
        double speific_rhoB      = specific_rhob0      + dt*k_rhoB;
        double speific_rhoS      = specific_rhos0      + dt*k_rhoS;
        double speific_rhoQ      = specific_rhoQ0      + dt*k_rhoQ;
        double bulk               = bigBulk0               + dt*kbigBulk;
        double particles_Ez       = particles_E0        + dt*kE0;
        milne::Vector<double,D> r = r0                  + dt*kr;
        milne::Vector<double,D> u = u0                  + dt*ku;
        milne::Matrix<double,2,3> bigshv;
        for(int idir=0; idir<2; ++idir){
        for(int jdir=idir; jdir<3; ++jdir){
          bigshv(idir,jdir)          = bigshv0(idir,jdir) + dt*kbigshv(idir, jdir);
        }
        }
        ...
        device_specific_density.access(is, ia, densities_info::rhoB) = speific_rhoB;
        device_specific_density.access(is, ia, densities_info::rhoS) = speific_rhoS;
        device_specific_density.access(is, ia, densities_info::rhoQ) = speific_rhoQ;
        ...
        for (int idir=0; idir<2; ++idir)
        for (int jdir=idir; jdir<3; ++jdir)
            device_hydro_shear_aux_vector.access(is, ia, hydro_info::bigshv, idir, jdir) = bigshv(idir,jdir);
...
void Evolver<D>::step_rk(double dt, double t0, std::function<void(void)> time_derivatives_functional ){
  ...
  auto update_rk2_step = KOKKOS_LAMBDA(const int is, const int ia){
    ...
    double particles_E0 = slice_E0.access(is, ia);
    milne::Matrix<double, 2, 3> bigshv0;
    ...
    //Cache derivatives
    double dbigBulk_dt = device_hydro_scalar.access(is,ia,hydro_info::dbigBulk_dt);
    ...
    double d_dt_specific_rhoB = device_d_dt_spec.access(is, ia, densities_info::rhoB);
    double d_dt_specific_rhoS = device_d_dt_spec.access(is, ia, densities_info::rhoS);
    double d_dt_specific_rhoQ = device_d_dt_spec.access(is, ia, densities_info::rhoQ);
    ...
    for(int idir=0; idir<2; ++idir){
    for(int jdir=idir; jdir<3; ++jdir){
      d_bigshv_dt(idir, jdir) = device_hydro_shear_aux_vector.access(is, ia, hydro_info::d_bigshv_dt, idir, jdir);
    }
    }

    //Compute updated quantities
    double specific_s         = specific_s0         + dt*d_dt_specific_s;
    double bigBulk               = bigBulk0               + dt*dbigBulk_dt;
    double rhoB                 = rhoB0                 + dt*d_dt_specific_rhoB;
    double rhoS                 = rhoS0                 + dt*d_dt_specific_rhoS;
    double rhoQ                 = rhoQ0                 + dt*d_dt_specific_rhoQ;
    milne::Vector<double,D> r = r0                  + dt*v;
    milne::Vector<double,D> u = u0                  + dt*du_dt;
    milne::Matrix<double,2,3> bigshv;
    for(int idir=0; idir<2; ++idir){
    for(int jdir=idir; jdir<3; ++jdir){
      bigshv(idir,jdir)          = bigshv0(idir,jdir) + dt*d_bigshv_dt(idir, jdir);
      //std::cout << "bigshv(" << idir << "," << jdir << ") = " << bigshv(idir,jdir) << std::endl;
      //std::cout << "bigshv0(" << idir << "," << jdir << ") = " << bigshv0(idir,jdir) << std::endl;
      //std::cout << "d_bigshv_dt(" << idir << "," << jdir << ") = " << d_bigshv_dt(idir,jdir) << std::endl;
    }
    }
    //Update in memory
    int freeze = device_freeze.access(is, ia); //Check if the cell is frozen
    ...
    device_specific_density.access(is, ia, densities_info::s) = specific_s;
    device_hydro_scalar.access(is, ia, hydro_info::bigBulk) = bigBulk;
    device_specific_density.access(is, ia, densities_info::rhoB) = rhoB;
    device_specific_density.access(is, ia, densities_info::rhoS) = rhoS;
    device_specific_density.access(is, ia, densities_info::rhoQ) = rhoQ;
    for (int idir=0; idir<D; ++idir){
      device_position.access(is, ia, idir) = r(idir);
      device_hydro_vector.access(is, ia, hydro_info::u, idir) = u(idir);
    }
