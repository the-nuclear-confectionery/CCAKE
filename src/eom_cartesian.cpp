...
static bool get_sorted_eigenvalues_of_pi_mu_nu(cartesian::Matrix<double, 4, 4> &shv_hybrid,
                                                 double &Lambda_0,
...
// /// @param sysPtr A shared pointer to the system state.
// template <unsigned int D>
// void EoM_cartesian<D>::check_causality(std::shared_ptr<SystemState<D>> sysPtr)
// {
//     double t = (sysPtr->t);
//     double t2 = t * t;
//     CREATE_VIEW(device_, sysPtr->cabana_particles);
...
//             necessary_conditions &= (condition4e && condition4f);
//         }
...
//               ( (lambda_Pipi / tau_Pi + cs2 - tau_pipi / (12. * tau_pi)) * (Lambda_3 + fabs(Lambda_1)) * (Lambda_3 + fabs(Lambda_1)) ) / 
...
//               ( (lambda_Pipi / tau_Pi + cs2 - tau_pipi / (12. * tau_pi)) * (Lambda_3 + fabs(Lambda_1)) * (Lambda_3 + fabs(Lambda_1)) ) /
//               ( (1.0 / (2. * tau_pi)) * (2 * eta + lambda_piPi * Pi) - (tau_pipi / (2. * tau_pi)) * fabs(Lambda_1)) /
//               * ( (1.0 / (2. * tau_pi)) * (2 * eta + lambda_piPi * Pi) - (tau_pipi / (2. * tau_pi)) * fabs(Lambda_1)) && 
...
void EoM_cartesian<D>::check_causality(std::shared_ptr<SystemState<D>> sysPtr)
{
    ...
    // ------------------------------------------------------------------
    // Host precompute of eigenvalues (GSL) with Kokkos::View + host mirror
    // ------------------------------------------------------------------
    const int S  = sysPtr->cabana_particles.size();
    constexpr int VL = VECTOR_LENGTH;
    const int N  = S * VL;

    // Temp device View to gather shv for each lane into a linear idx
    Kokkos::View<double***> shv_d("ccake_causality_shv_d", N, 4, 4);

    // Fill shv_d on device from device_hydro_spacetime_matrix
    {
        auto fill_shv = KOKKOS_LAMBDA(const int is, const int ia) {
        ...
        Cabana::simd_parallel_for(simd_policy, fill_shv, "gather_shv_for_eigs");
        Kokkos::fence();
    }

    // Host mirror + eigenvalues on host
    auto shv_h = Kokkos::create_mirror_view(shv_d);
    Kokkos::deep_copy(shv_h, shv_d);

    Kokkos::View<double*> L1("ccake_causality_L1", N);
    Kokkos::View<double*> L2("ccake_causality_L2", N);
    Kokkos::View<double*> L3("ccake_causality_L3", N);
    Kokkos::View<int*>    ok("ccake_causality_ok", N);
    ...
    for (int idx = 0; idx < N; ++idx) {
        cartesian::Matrix<double, 4, 4> shv, shv_hybrid;
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
        ...
        double Lambda_0, Lambda_1, Lambda_2, Lambda_3;
        bool good = get_sorted_eigenvalues_of_pi_mu_nu(shv_hybrid,
    ...
    // ----------------------------------------------------
    // Device kernel: read precomputed eigenvalues and test
    // ----------------------------------------------------
    auto causality_check = KOKKOS_LAMBDA(const int is, const int ia)
    {
        const int idx = is * VL + ia;

        // bail out if eigen-solve failed for this lane
        if (!ok(idx)) {
            // keep your cout (host-only, device ignore)
        ...
        // pull eigenvalues from precompute
        double Lambda_1 = L1(idx);
        double Lambda_2 = L2(idx);
        double Lambda_3 = L3(idx);

        // use plain C array (avoid std::array operator[] device constexpr issue)
        ...
        for (int a = 1; a <= 3; ++a) {
            for (int d = 1; d <= 3; ++d) {
                if (a == d) continue;
                double Lambda_a = Lambda[a - 1];
                double Lambda_d = Lambda[d - 1];

                bool condition4c = (1.0 / (2. * tau_pi)) * (2. * eta + lambda_piPi * Pi)
                                 + (tau_pipi / (4. * tau_pi)) * (Lambda_a + Lambda_d) >= 0;

                bool condition4d = (e + p + Pi + Lambda_a
                                   - (1.0 / (2. * tau_pi)) * (2. * eta + lambda_piPi * Pi)
                                   - (tau_pipi / (4. * tau_pi)) * (Lambda_d + Lambda_a)) >= 0;
        ...
        if (!necessary_conditions) {
            ...
            bool sufficient_conditions =
                ...
                + ((12. * delta_pipi - tau_pipi) / (12. * tau_pi)) *
                  ((lambda_Pipi / tau_Pi + cs2 - tau_pipi / (12. * tau_pi)) *
                   (Lambda_3 + fabs(Lambda_1)) * (Lambda_3 + fabs(Lambda_1))) /
                  ((e + p + Pi - fabs(Lambda_1))
                    - (1.0 / (2. * tau_pi)) * (2. * eta + lambda_piPi * Pi)
            ...
            causality_result = sufficient_conditions ? 1 : 0; // 1: causal, 0: undetermined
        }
