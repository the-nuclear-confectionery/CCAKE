...
AVG_TIME_FILE="cpu_time.log"     # File to store the average time
TIME_SUM=0                           # Sum of times for all runs
TIMESTEP_SUM=0                       # Sum of timesteps for all runs
OUTPUT_DIR_BASE="../build/output_visc_gubser/cpu/sim"             # Base directory for outputs
...
for i in $(seq 1 $NUM_RUNS); do
    ...
    srun --output=out_cpu_${i}.out --error=err_cpu_${i}.err make run-visc-gubser OUTPUT_DIR="${OUTPUT_DIR_BASE}_$i"
