...
public:
  ...
  /// is provided which does not need to take into account cartesian components.
...
void Matrix<T,D1,D2>::make_covariant(int slot, double t2){
  ...
  switch (slot){
    case 0: ///Lower first index.
      ...
      for (int j=0; j<D2; ++j) x[index(D1-1,j)] *= -1.;
      break;
    case 1:
      ...
      // M_i^3 = g_{33} M^{i3}
      for (int i=0; i<D1; ++i) x[index(i,D2-1)] *= -1.;
      break;
...
void Matrix<double,4,4>::make_covariant(int slot, double t2){
  ...
  switch (slot){
    case 0: ///Lower first index.
      ...
      for (int j=0; j<4; ++j) x[index(3,j)] *= -1.;
    ...
    case 1:
      ...
      for (int i=0; i<4; ++i) x[index(i,3)] *= -1.;
