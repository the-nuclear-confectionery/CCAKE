...
# set up
data=np.loadtxt('../Thermodynamics/EoS_Taylor_AllMu.dat', usecols=(0,1,2,3,6,7,8,9))
...
np.savetxt('grid_ebsq_mag.dat', grid, fmt=("%d" + " %12.8f"*8))
#np.savetxt('hull_ebsq_mag.dat', grid[hull.vertices], fmt=("%d" + " %12.8f"*8))
...
np.savetxt('grid_logebtstqt_mag.dat', grid, fmt=("%d" + " %12.8f"*8))
np.savetxt('hull_logebtstqt_mag.dat', grid[hull.vertices], fmt=("%d" + " %12.8f"*8))
...
hull = ConvexHull(np.c_[ e, b, s, q ])
grid=np.c_[range(len(T)),T,muB,muS,muQ,e,b,s,q]
np.savetxt('grid_ebsq_norm.dat', grid, fmt=("%d" + " %12.8f"*8))
np.savetxt('hull_ebsq_norm.dat', grid[hull.vertices], fmt=("%d" + " %12.8f"*8))

print('hull = ConvexHull(np.c_[ loge, bt, st, qt ]); norm')
hull = ConvexHull(np.c_[ loge, bt, st, qt ])
grid=np.c_[range(len(T)),T,muB,muS,muQ,loge,bt,st,qt]
np.savetxt('grid_logebtstqt_norm.dat', grid, fmt=("%d" + " %12.8f"*8))
np.savetxt('hull_logebtstqt_norm.dat', grid[hull.vertices], fmt=("%d" + " %12.8f"*8))
